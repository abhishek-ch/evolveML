...
def index_corpus():
	...
	d = defaultdict(int)
	for e, row in enumerate( csv.DictReader(open(BASE_DATA_PATH+"/train.csv",'r', newline='', encoding='utf8'))):
		s = clean("medianrellabel%s %s %s"%(row["median_relevance"], row["product_description"],row["product_title"]))
		d = index_document(s,d)
	for e, row in enumerate( csv.DictReader(open(BASE_DATA_PATH+"/test.csv",'r', newline='', encoding='utf8'))):
