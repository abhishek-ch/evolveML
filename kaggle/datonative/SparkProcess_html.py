...
def parse_pageDataframe(page, urlid):
    ...
    try:
        soup = bs(page, 'html.parser')
        output = Row(id=urlid, text=parse_text(soup), title=parse_title(soup), links=parse_links(soup),
                     images=parse_images(soup))
...
if __name__ == "__main__":
    # conf = (SparkConf().setMaster("local[*]").setAppName("Process HTML2"))
    sc = SparkContext()
    sqlContext = SQLContext(sc)
    stopwords = set(nltk.corpus.stopwords.words('english'))
    stopwords = sc.broadcast(stopwords)

    maindir = '/Volumes/work/data/kaggle/dato/'
