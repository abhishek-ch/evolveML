...
sampleDistinctFeats = (sampleDataRDD
                       .flatMap(lambda value:value).distinct())
...
def createOneHotDict(inputData):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    return (inputData.flatMap(lambda value:value).distinct()).zipWithIndex().collectAsMap()
...
def parsePoint(point):
    ...
    return [(featureID,value) for (featureID,value) in enumerate(values)]
...
# TODO: Replace <FILL IN> with appropriate code
model0 = LogisticRegressionWithSGD.train(OHETrainData,iterations=numIters,step=stepSize,regParam=regParam,regType=regType,
